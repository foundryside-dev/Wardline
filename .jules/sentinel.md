...
## 2026-06-21 - [Add Unsafe Third-Party Loaders to Taint Tracking]
**Vulnerability:** The static analyzer was missing third-party deserialization functions (like `dill.load`, `dill.loads`, `jsonpickle.decode`, `joblib.load`, `torch.load`) in its `_SERIALISATION_SINKS` mapping, leading to false negatives because the analyzer failed to shed validation provenance and convert their outputs to `UNKNOWN_RAW`.
**Learning:** All deserialization sinks specified in rules (e.g., `_SINK_SPECS` in `untrusted_to_deserialization.py`) must also be registered in the core taint propagation logic (`_SERIALISATION_SINKS`). The exception is conditionally unsafe sinks like `numpy.load` which should NOT be added to avoid false positives in safe-by-default configurations.
**Prevention:** When adding new sinks to rule definitions, always verify if they need to be added to core propagation mappings like `_SERIALISATION_SINKS`.
