...
## 2025-02-14 - Add Unsafe Deserialization Loaders to Taint Tracking
...
**Learning:** Even if functions are listed in rule specifications (like `_SINK_SPECS`), they also need to be properly categorized in the core taint propagation logic (`_SERIALISATION_SINKS`) to ensure the analyzer correctly sheds validation provenance (converting output to `UNKNOWN_RAW`).
**Prevention:** When adding new deserialization functions to scanner rules, ensure they are also added to core propagation mappings like `_SERIALISATION_SINKS`. Exceptions like conditionally unsafe sinks (e.g. `numpy.load` which requires `allow_pickle=True`) should be omitted to avoid false positives.
